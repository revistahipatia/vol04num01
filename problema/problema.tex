...
Como a sequência é limitada inferiormente por $\sqrt{c}$ e ela é decrescente a partir do primeiro termo $n\geq1$, ela converge.
...
Seja $A$ uma matriz quadrada de ordem $n$ ($n \ge 2$). Se a uma fila (linha ou coluna) de $A$ for adicionado um múltiplo escalar de outra fila paralela a ela, obtemos uma matriz $B$ tal que:
$$\det(A) = \det(B)$$
...
$$a''_{11} = 2 - \sum_{k=2}^n (-1)^k a_{1k} = 2 - \sum_{k=2}^n (-1)^k (-1)^{k-1} = 2 - \sum_{k=2}^n (-1)^{2k-1}$$
...
O determinante de uma matriz triangular é dado pelo produto dos elementos de sua diagonal principal:
...
-1 & 2 & -1 \\ 
1 & -1 & 2
\end{bmatrix} \ \mbox{e} \ A_4 = \begin{bmatrix}
2 & -1 & 1 & -1\\
-1 & 2 & -1 & 1 \\ 
1 & -1 & 2 & -1 \\
-1 & 1 & -1 & 2
...
Para calcular o determinante de $A_n$ notemos que, ao realizar operações elementares sobre as linhas do tipo $L_i \leftarrow L_i + L_j$, $i \neq j$, mantemos o determinante da matriz. Agora, veja que, fazendo a sequência de operações dadas a seguir a partir de $A_n$, $$L_1 \leftarrow L_1 + L_2$$ $$L_2 \leftarrow L_2 + L_3$$ $$\vdots$$ $$L_{n-1} \leftarrow L_{n-1} + L_n$$ obtemos a matriz $$\overline{A}_n = \begin{bmatrix}
1 & 1 & 0 & 0 & \cdots & 0 & 0 & 0\\
0 & 1 & 1 & 0 & \cdots & 0 & 0 & 0 \\
\vdots & \vdots & \vdots & \vdots & \ddots & \vdots & \vdots & \vdots\\
0 & 0 & 0 & 0 & \cdots & 0 & 1 & 1 \\
(-1)^{n-1} & (-1)^{n-2} & (-1)^{n-3} & (-1)^{n-4} & \cdots & 1 & -1 & 2
\end{bmatrix}$$ que possui o mesmo determinante de $A_n$. Agora, calculamos o determinante de $\overline{A}_n$ pelo o Teorema de Laplace, usando a última linha. 

Veja que, as submatrizes de $\overline{A}_n$ obtidas ao eliminarmos a linha $n$ e coluna $k$, são matrizes triangulares com a diagonal principal com todas as entradas iguais a 1, e isso implica que os determinantes dessas submatrizes são todos iguais a 1. Portanto, \begin{align*}
    \det A_n = \det \overline{A}_n =& (-1)^{n-1} \cdot (-1)^{n+1} \cdot 1 + (-1)^{n-2} \cdot (-1)^{n+2} \cdot 1 + \cdots\\ & + 1 \cdot (-1)^{n+n-2} \cdot 1 + (-1) \cdot (-1)^{n+n-1} \cdot 1 + 2 \cdot (-1)^{n+n} \cdot 1 \\ =& 1 + 1 + \cdots + 1 + 1 + 2 = (n-1) + 2 = n+1
\end{align*}
...
Os pontos da esfera que são projetados nessa região (aqui contando as partes superior e inferior da esfera que se projetam na região poligonal) formam uma região cuja área pode ser calculada através de $$A = 4\pi - 5 \cdot A_{\mbox{revolução}}$$ onde: $4\pi$ é a área da superfície esférica de raio 1 ($4\pi R^2$), e $A_{\mbox{revolução}}$ é a área da superfície de revolução obtida pela rotação da curva que pode ser parametrizada por $$\mathcal{C} : \begin{cases}
    x(t) = \cos(t), \\ y(t) = \sin(t)
\end{cases} \ \ \ 0 \leq t \leq \frac{\pi}{5}$$ (cujo traço é o arco em destaque na figura), em torno do eixo $x$. Temos que, \begin{align*}
...
[O fato de que $\cos(\pi/5) = \frac{1+\sqrt{5}}{4}$ pode ser obtido usando relações trigonométricas em um triângulo isosceles de ângulos $36^{\circ}, 72^{\circ}, 72^{\circ}$ e base 1]. Portanto, $$A = \frac{8\pi - 5(3 - \sqrt{5})\pi}{2} = \frac{(5\sqrt{5} - 7)\pi}{2}$$
...
A demonstração utiliza a Desigualdade das Médias Aritmética e Geométrica (MA-MG). Para quaisquer números reais positivos $x$ e $y$, temos:
...
\item Para $n=3$: por simetria, podemos considerar a divisão inicial $3 = 2+1$, anotando no quadro o valor $2$. Após isso, veja que os próximos números a serem anotados no quadro são os que podem ser gerados começando com $2$ pedras. E nesse caso já sabemos o que ocorre. Assim, ao final teremos a soma $S_3 = 2 + 1 = 3$;

\item Para $n=4$: podemos começar com duas possíveis divisões iniciais $4 = 2+2$ ou $4=3+1$, anotando no quadro o valor $4$ ou $3$, respectivamente. Após isso, veja que os próximos números a serem anotados no quadro são os que podem ser gerados começando com $2$ pedras ou $3$ pedras. E nesse caso já sabemos o que ocorre. Assim, ao final teremos a soma $S_4 = 4 + 1 + 1 = 6$ (no primeiro caso) e $S_4 = 3 + 3 = 6$ (no segundo caso). Podemos ver que de fato, a soma final $S_n$ parece não depender das escolhas de divisões feitas;
...
Pronto, agora vamos provar em definitivo que de fato, isso independe das escolhas de divisões inicial e intermediárias. Provaremos, por indução, que $S_n = \frac{n(n-1)}{2}$ independe das escolhas de divisões. 
...
Pela conclusão anterior, sem perda de generalidade, pode-se inferir que PX é perpendicular à reta suporte de EM, chamemos ela de r. Analogamente, PY é perpendicular à reta suporte de EN, chamemos ela de s.
...
\begin{center}
$\angle XFY + 90° + 90° + \angle YPX=360°$
\end{center}

\vspace{0.2cm}

\begin{center}
$\Leftrightarrow \angle YPX=180°- \angle XFY$
\end{center}
...
Como visto anteriormente, $PX \perp r \supset EM$ e $PY \perp s \supset EN$, então obrigatoriamente, $MN \perp XY$. Isso é comprovado pelo fato de que como a relação de semelhança consiste em realizar no máximo rotações e translações, tem-se que ao realizar algum desses movimentos de tal forma que dois pares de lados homólogos sejam perpendiculares entre si, então o terceiro par também será perpendicular.
...
\begin{problem*}
Qual o primeiro algarismo após a vírgula da representação decimal de $\sqrt{9 \cdot 100^2+4 \cdot 100}$?
\end{problem*}


\begin{problem*}
A função $f: \mathbb{N} \rightarrow \mathbb{R}$ satisfaz, $f(1)=2027$ e, para $n>1$, $$f(1)+f(2)+\ldots +f(n) = n^2 \cdot f(n)$$ Qual o valor de $f(2027)$?
...
\begin{enumerate}
\item[a)] Qual o número máximo de jogadas que Elaís pode fazer na palavra $LEELEELLE$?

\item[b)] Dentre todas as palavras de $20$ letras, qual o valor máximo possível de jogadas que ela pode fazer?
\end{enumerate}
\end{problem*}
...
\begin{problem*} Suponha que $x$ e $y$ são maiores que $0$. Vamos denotar o mínimo de $x, \frac{1}{y}$ e $y + \frac{1}{x}$ por $S$, isto é, $S = \min\{ x, \frac{1}{y}, y + \frac{1}{x}\}$. Qual o valor máximo possível para $S$ ?
\end{problem*}
