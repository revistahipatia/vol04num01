...
\theoremstyle{remark}
\newtheorem{remark}[theorem]{\bf Observação}
...
\section{Exemplos introdutórios de bilhares dinâmicos}
...
\end{figure}
Em ambos os casos, é fácil notar pelo teorema de Pitágoras que as cordas determinadas por uma trajetória são tangentes a um mesmo círculo de raio $\sqrt{1-\ell^2}$ e concêntrico ao original. 
...
Ao contrário do círculo, as retas normais à elipse já não passam todas por um mesmo ponto.
...
são múltiplos racionais de $\pi$. Neste caso, o grupo de simetria do polígono é finito,
...
O matemático russo-americano Yakov Sina\u{\i} iniciou, no final da década de 1960, um estudo sistemático de bilhares caóticos.
...
Sempre que um feixe de trajetórias paralelas se choca com o bordo, ele se dispersa.
Portanto, se imaginarmos um conjunto de trajetórias inicialmente bem próximas, após algumas batidas elas ficam praticamente independentes, espalhando-se por toda a mesa, indicando uma {\bf sensibilidade às condições iniciais}.
\begin{figure}[h!]
\centering
...
ser construídos no toro bidimensional $\mathbb T^2$: considerando um domínio fundamental igual a um quadrado de lado 1, considere dois obstáculos circulares, um deles localizado no centro do quadrado e o outro em um dos vértices do quadrado, como na figura abaixo.
\begin{figure}[h!]
\centering
\includegraphics[width=6cm]{fig97_repro.png}
\end{figure}
...
\begin{figure}[h!]
\centering
\includegraphics[width=7cm]{fig109_repro.png}
...
M. Viana e K. Oliveira, \emph{Fundamentos da Teoria Ergódica}.
